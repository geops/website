...
export default function getContentItem(language, collection, path, fields) {
  ...
  if (fs.existsSync(join(itemPath, "index.de.md"))) {
    ...
    // Collect image fields from frontmatter to include in imageSizes
    const imageFields = [
      data.gridImage,
      data.cover,
      data.frontpageImage,
      ...
      data.logo,
      data.photo,
      // add more image paths from content.sections.image if needed
      ...(data.content?.sections || []).map((section) => section.image),
      ...Array.from(
        data.content?.lead?.matchAll(markdownImagePathRegExp) || [],
        (m) => m[1],
      ),
    ].filter(Boolean);
