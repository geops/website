...
const imagePathRegExp = /!\[\]\((\/images\/[a-z/_\-\.]*)\)/g;
function getImageSizes(content) {
  const imageSizes = {};
  const imagePaths = Array.from(content.matchAll(imagePathRegExp), (m) => m[1]);
  imagePaths.forEach((path) => {
    imageSizes[path] = imageSize(join(process.cwd(), "public", path));
  });
  return imageSizes;
}
